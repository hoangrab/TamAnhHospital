...
export class DoctorsComponent {
  ...
  constructor(private route: ActivatedRoute,private doctorsv: DoctorService, 
    private formbuilder: FormBuilder,private majorsv : MajorService, private changref: ChangeDetectorRef) { };
  ...
  ngOnInit() {
    this.addForm = this.formbuilder.group({
      ...
      phone: ['', Validators.compose([Validators.required, Validators.minLength(9), Validators.maxLength(12)])],
    ...
    this.route.params.subscribe(params => {
      ...
      this.title = activeParam === 'true' ? 'Danh sách bác sĩ đang làm' : 'Danh sách bác sĩ đã rời';
  ...
  onsave() {
    ...
    if (this.addForm.valid) {
      const {fullName,userName,password,phone,email,description,majorId} = this.addForm.value;
      const formda = new FormData();
      if (this.fileanh1 !== null) {
        formda.append('file', this.fileanh1);
      }
      const encoder = new TextEncoder();
      const utf8Array = encoder.encode(JSON.stringify({fullName,userName,password,phone,email,description,majorId}));
  ...
  onupdate() {
    this.submited = true;
    const {fullName,userName,password,phone,email,description,majorId,trangthai} = this.addForm.value;
    if (!this.addForm.controls['fullName'].errors && !this.addForm.controls['userName'].errors
    && !this.addForm.controls['phone'].errors&& !this.addForm.controls['email'].errors
    && !this.addForm.controls['description'].errors) {
      ...
      const encoder = new TextEncoder();
      const utf8Array = encoder.encode(JSON.stringify({fullName,userName,password,phone,email,description,majorId,trangthai}));
      const binaryString = String.fromCharCode(...utf8Array);
      const base64Encoded = btoa(binaryString);
      formda.append('doctorDto',base64Encoded)
  ...
  xoa(id: number){
    this.doctorsv.delete(id).subscribe({
     next:(value) => {
        this.loaddata()
        alert(value.message)
     }, 
